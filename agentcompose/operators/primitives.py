...
class SmartPrimitive:
    """Wraps a transformation function to enable partial application.

    SmartPrimitive handles both single-argument and multi-argument transformations:
    ...
    def __call__(self, *cols, **kwargs):  # type: ignore
        ...
        Returns:
            If columns provided: The transformed Any
            If no columns: A configured function that takes Any(s)
        """
        if cols:
        ...
        **namespaces,
        ...
        def decorator(func: Callable):
            ...
            if steps is not None:
                ...
                def pipeline(col: Any) -> Any:  # type: ignore
...
class StablePipeline:
    ...
    def __call__(self, col: Any) -> Any:  # type: ignore
        """Execute the pipeline"""
        return self._execute_steps(self.steps, col)  # type: ignore

    def _execute_steps(self, steps: List[CompiledStep], col: Any) -> Any:  # type: ignore
        result = col  # type: ignore
...
class AgentRegistry(PrimitiveRegistry):
    """The initial step for creating a resource for an AI Agent that can be used."""
...
__all__ = ["SmartPrimitive", "PrimitiveRegistry", "AgentRegistry"]
